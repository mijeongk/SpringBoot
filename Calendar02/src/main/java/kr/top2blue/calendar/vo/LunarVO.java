...
public class LunarVO {
	...
	// 1965년 11월 09일
	// 2023년 윤02월 01일
	public int getLunarYear() { // 음력 년도만 얻기
		return Integer.parseInt(lunar.substring(0,4));		
	}
	public String getLunarMonth() { // 음력 월만 얻기
		// split(" ") : 구분자를 기준으로 잘라서 배열로 만든다.
		String m = lunar.split(" ")[1];
		return m.substring(0,m.length()-1);		
	}
	public int getLunarDate() { // 음력 일만 얻기
		return Integer.parseInt(lunar.split(" ")[2].substring(0,2));		
	}
	
	// 간지를 세차/월건/일진 으로 각각 구분하여 가져오기
	// 평달의 경우 : 계묘(癸卯)년 을묘(乙卯)월 무인(戊寅)일
	// 윤달의 경우 : 계묘(癸卯)년 기묘(己卯)일    ===>  윤달은 월건이 없다.
	// 년도를 얻어보자
	// 한글
	...
	public String getDayOfWeekS() {
		return dayOfWeek.substring(0,1); // 일, 월, 화 ... 토
	}
	// 요일을 숫자로 얻는 메서드
